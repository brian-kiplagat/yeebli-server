...
export class Server {
  ...
  public configure() {
    ...
    const leadController = new LeadController(leadService, userService, eventService);
  ...
  private registerStripeRoutes(api: Hono, stripeCtrl: StripeController) {
    ...
    stripe.post('/connect/account', authCheck, stripeUrlValidator, stripeCtrl.createConnectAccount);
    stripe.get('/connect/status', authCheck, stripeCtrl.getAccountStatus);
    stripe.post('/webhook', stripeWebhookValidator, stripeCtrl.handleWebhook);
