...
export class TeamController {
  ...
  public inviteMember = async (c: Context) => {
    try {
      ...
      if (existingUser) {
        ...
        const existingInvitations = await this.service.repo.getInvitationsByEmail(invitee_email);
      ...
      const invitation = await this.service.inviteMember(team.id, user.id, invitee_email, team.name);
  ...
  public getMyTeamMembers = async (c: Context) => {
    try {
      ...
      const members = await this.service.repo.getTeamMembers(teamMember.team_id);
      if (!members) {
  ...
  public revokeAccess = async (c: Context) => {
    try {
      ...
      const isMember = teamMembers.some((member) => member.user_id === member_id);
      if (!isMember) {
