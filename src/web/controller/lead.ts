...
export class LeadController {
  ...
  constructor(service: LeadService, userService: UserService, eventService: EventService) {
  ...
  public handleExternalForm = async (c: Context) => {
    try {
      ...
      const eventTimeGMT = new Date(Number(event.event_date) * 1000).toLocaleString('en-GB', {
        timeZone: 'UTC',
        year: 'numeric',
        month: 'long',
        day: '2-digit',
